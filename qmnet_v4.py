...
# ============================================================
# SECTION 7: BRIDGE TOPOLOGY SWEEP
# ============================================================

@dataclass
class BridgeSweepResult:
    bridge_pair: Tuple[int, int]
    bridge_in_graph: bool        # True if this edge already exists in base graph
    T: int
    p_ret_m0: float              # P(return) when bridge NOT fired
    p_ret_m1: float              # P(return) when bridge WAS fired
    zk_m0: Optional[float]
    zk_m1: Optional[float]
    zkzl_m0: Optional[float]
    zkzl_m1: Optional[float]
    n_m0: int
    n_m1: int

    @property
    def delta_p_ret(self) -> float:
        """P_ret(m=0) - P_ret(m=1): positive means bridge hurts echo return."""
        return self.p_ret_m0 - self.p_ret_m1

    @property
    def delta_zk(self) -> Optional[float]:
        """⟨Z_k⟩(m=0) - ⟨Z_k⟩(m=1): polarization shift caused by bridge."""
        if self.zk_m0 is None or self.zk_m1 is None:
            return None
        return self.zk_m0 - self.zk_m1


def run_bridge_topology_sweep(
    n_qubits: int = 5,
    graph_edges: Optional[List[Tuple[int, int]]] = None,
    bridge_pairs: Optional[List[Tuple[int, int]]] = None,
    T_list: Optional[List[int]] = None,
    rule_qubit: int = 1,
    W_qubit: int = 0,
    W_op: str = "z",
    shots: int = 4096,
) -> List[BridgeSweepResult]:
    """
    Sweep over multiple bridge pairs and compare their effect on echo return.

    Research question:
      Does the topological position of the bridge (hub vs leaf vs cross)
      determine how strongly it disrupts the echo?

    For a star graph (qubit 0 as hub):
      (0, 4) — hub to leaf: bridge touches the most-connected qubit
      (1, 2) — leaf to leaf: bridge between peripheral qubits
      (2, 3) — leaf to leaf (adjacent leaves)
      (1, 3) — leaf to leaf (non-adjacent leaves)
      (0, 1) — edge IN the base graph: bridge re-applies an existing edge

    Expected: hub bridges more disruptive than leaf bridges.
    If confirmed → topology of the graph determines bridge sensitivity.

    Args:
        bridge_pairs : list of (i,j) pairs to test; default covers all cases above
        T_list       : scrambler depths; default [2, 4, 6] (avoids T=0 trivial case)
        (other args same as run_bridge_experiment)

    Returns:
        List of BridgeSweepResult, one per (bridge_pair, T) combination.
        Also prints a summary table sorted by delta_p_ret.
    """
    if not _HAS_AER:
        print("ERROR: qiskit-aer not found.")
        return []

    if graph_edges is None:
        graph_edges = [(0, 1), (0, 2), (0, 3), (0, 4)]  # star

    if bridge_pairs is None:
        bridge_pairs = [
            (0, 4),   # hub → leaf (maximally disruptive?)
            (1, 2),   # leaf → leaf (adjacent)
            (2, 3),   # leaf → leaf (adjacent, different)
            (1, 3),   # leaf → leaf (non-adjacent)
            (0, 1),   # IN base graph (bridge re-applies existing edge)
        ]

    if T_list is None:
        T_list = [2, 4, 6]   # skip T=0 (trivial) and T=1 (oscillation artifact)

    graph_edge_set = {(min(u, v), max(u, v)) for u, v in graph_edges}
    sim = AerSimulator()
    all_results: List[BridgeSweepResult] = []

    print("\n" + "=" * 78)
    print("BRIDGE TOPOLOGY SWEEP")
    print(f"Base graph : {graph_edges}  (star, qubit 0 = hub)")
    print(f"Rule qubit : {rule_qubit}  |  Perturbation: {W_op.upper()} on q{W_qubit}")
    print(f"Shots: {shots}  |  T values: {T_list}")
    print("=" * 78)
    print(
        f"{'bridge':>8} {'in_graph':>9} {'T':>3} | "
        f"{'P_ret|m0':>9} {'P_ret|m1':>9} {'Δ P_ret':>8} | "
        f"{'<Zk>|m0':>8} {'<Zk>|m1':>8} {'Δ<Zk>':>8} | "
        f"{'<ZkZl>|m0':>10} {'<ZkZl>|m1':>10}"
    )
    print("-" * 78)

    for bridge_pair in bridge_pairs:
        k, l = bridge_pair
        in_graph = (min(k, l), max(k, l)) in graph_edge_set
        ...
        for T in T_list:
            ...
            r = BridgeSweepResult(
                bridge_pair=bridge_pair,
                bridge_in_graph=in_graph,
                T=T,
                p_ret_m0=p0, p_ret_m1=p1,
                zk_m0=zk0,   zk_m1=zk1,
                zkzl_m0=zkzl0, zkzl_m1=zkzl1,
                n_m0=n0, n_m1=n1,
            )
            ...
            delta_p = r.delta_p_ret
            delta_z = r.delta_zk

            print(
                f"{str(bridge_pair):>8} {'YES' if in_graph else 'no':>9} {T:>3} | "
                f"{p0:>9.4f} {p1:>9.4f} {delta_p:>+8.4f} | "
                f"{f(zk0):>8} {f(zk1):>8} {f(delta_z):>8} | "
                f"{f(zkzl0):>10} {f(zkzl1):>10}"
            )

        print()  # blank line between bridge pairs

    # Summary: rank bridge pairs by mean |Δ P_ret| across T values
    print("=" * 78)
    print("SUMMARY — mean |ΔP_ret| per bridge pair (higher = more disruptive)")
    print("-" * 78)

    from collections import defaultdict
    delta_by_pair: Dict[Tuple[int,int], List[float]] = defaultdict(list)
    for r in all_results:
        delta_by_pair[r.bridge_pair].append(abs(r.delta_p_ret))

    ranked = sorted(delta_by_pair.items(), key=lambda x: np.mean(x[1]), reverse=True)
    for pair, deltas in ranked:
        in_g = (min(pair), max(pair)) in graph_edge_set
        mean_d = np.mean(deltas)
        bar = "█" * int(mean_d * 40)
        tag = " ← in base graph" if in_g else ""
        print(f"  {str(pair):>8}  |Δ|={mean_d:.4f}  {bar}{tag}")

    print()
    print("Interpretation:")
    print("  Δ P_ret = P_ret(m=0) − P_ret(m=1)")
    print("  Positive Δ: bridge (m=1) hurts echo return more than no bridge (m=0)")
    print("  Large |Δ|: this bridge position is topologically sensitive")
    print("  If hub bridges > leaf bridges: graph topology drives bridge sensitivity")
...
if __name__ == "__main__":
    ...
    # --- Experiment 1: single bridge pair, full T sweep + baseline ---
    ...
    # --- Experiment 2: topology sweep — does bridge position matter? ---
    run_bridge_topology_sweep(
        n_qubits=5,
        graph_edges=[(0, 1), (0, 2), (0, 3), (0, 4)],
        bridge_pairs=[
            (0, 4),   # hub → leaf
            (1, 2),   # leaf → leaf (adjacent)
            (2, 3),   # leaf → leaf (adjacent, different pair)
            (1, 3),   # leaf → leaf (non-adjacent)
            (0, 1),   # IN base graph (redundant edge)
        ],
        T_list=[2, 4, 6],
        rule_qubit=1,
        W_qubit=0,
